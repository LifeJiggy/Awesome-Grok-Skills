...
class RedTeamEngine:
    ...
    def __init__(self, engagement: RedTeamEngagement):
        ...
        self.infrastructure: Optional[RedTeamInfrastructure] = None
        self.attack_results: list[AttackResult] = []
        self.team: list[TeamMember] = []
        self.findings: list[Finding] = []
        self._config: dict[str, Any] = {}

    def configure(self, infrastructure: RedTeamInfrastructure | None = None,
                  opsec_level: OPSECLevel = OPSECLevel.HIGH) -> None:
        """Configure the red team engine."""
        self.infrastructure = infrastructure
        self._config = {"opsec_level": opsec_level.value}
    ...
    def get_status(self) -> dict[str, Any]:
        ...
        return {
            ...
            "objectives": {
                ...
                "completed": sum(
                    1 for o in self.engagement.objectives
                    if o.status == ObjectiveStatus.COMPLETED
                ),
            },
            "team_size": len(self.team),
            "active_sessions": sum(
                len(m.active_sessions) for m in self.team if m.is_active
            ),
            "attack_results": len(self.attack_results),
            "findings": len(self.findings),
            "infrastructure_deployed": (
                self.infrastructure.deployed if self.infrastructure else False
            ),
        }
...
class AttackChainManager:
    ...
    def create_initial_access_phase(
        self, technique: str = "T1566.001", vector: str = "phishing"
    ) -> AttackPhase:
    ...
    def create_lateral_movement_phase(
        self, parent_id: str, target_network: str = "10.0.0.0/16"
    ) -> AttackPhase:
    ...
    def get_sitrep(self) -> dict[str, Any]:
        ...
        return {
            ...
            "active_operators": sum(
                1 for m in self.engine.team if m.is_active
            ),
...
class OperationalReport:
    ...
    def generate_full_report(self) -> str:
        ...
        lines = [
            ...
            f"**Period:** {self.engine.engagement.start_date} to "
            f"{self.engine.engagement.end_date}",
            "",
            "## Executive Summary",
        ...
        for obj in self.engine.engagement.objectives:
            lines.append(
                f"- [{obj.status.name}] {obj.name}: {obj.description}"
            )
        ...
        if self.detection_analysis:
            lines.extend([
                "", "## Detection Analysis",
                f"**SOC Response Time:** "
                f"{self.detection_analysis.soc_response_time}",
                f"**IR Capabilities:** "
                f"{self.detection_analysis.ir_capabilities}",
                f"**Detection Coverage:** "
                f"{self.detection_analysis.detection_coverage:.1f}%",
                "",
                "**Monitoring Gaps:**",
            ])
...
def main() -> None:
    ...
    infra = RedTeamInfrastructure(
        ...
        c2_servers=[
            C2Server(
                ...
                host="c2.redteam.example.com",
            ),
        ],
        redirectors=[
            Redirector(
                host="redirector.redteam.example.com",
                upstream="c2.redteam.example.com:443",
            ),
        ],
    ...
    print(
        f"\nMITRE Matrix: "
        f"{json.dumps(report.generate_mitre_matrix(), indent=2)}"
    )
    ...
    # Risk assessment
    risk = calculate_engagement_risk(engine.findings)
    print(f"\nEngagement risk: {risk}")

    # Dashboard
    dashboard = OperationalDashboard(engine)
    sitrep = dashboard.get_sitrep()
    print(f"\nSitRep: {json.dumps(sitrep, indent=2)}")
