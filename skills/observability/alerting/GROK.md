...
- `alertmanager_alerts_received_total` — alerts received from rules.
- `alertmanager_silences_queries_total` — silence query operations.
...
```
Initial:  error_rate > 5% for 10m  → tune to → error_rate > 2% for 5m
```
...
evaluating every 10s gives 30 data points — sufficient resolution without
wasting compute.
...
- alert: Watchdog
  ...
  labels:
    severity: critical
  annotations:
    description: "Monitoring pipeline may be down — Watchdog alert fired"
```
